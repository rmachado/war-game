...
export default function Home() {
  ...
  async function handleJoin() {
    if (!name.trim() || !joinCode.trim()) { setError('Llena todos los campos'); return }
    setLoading(true)
    setError('')
    try {
      const data = await joinGame(joinCode.trim().toUpperCase(), name.trim(), color)
      setSession(data.code, data.token)
      navigate(`/game/${data.code}`)
    } catch (e: any) {
      setError(e.message)
  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-4">
      <div className="w-full max-w-md">
        <h1 className="text-4xl font-bold text-center mb-2 tracking-wider">WAR</h1>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-stone-400 mb-1">Color de ejército</label>
          ...
          {mode === 'create' && (
            <div>
              <label className="block text-sm text-stone-400 mb-1">Jugadores</label>
          ...
          {mode === 'join' && (
            <div>
              <label className="block text-sm text-stone-400 mb-1">Código de juego</label>
              <input
                type="text"
                value={joinCode}
          ...
          >
            {loading ? 'Cargando...' : mode === 'create' ? 'Crear juego' : 'Unirse al juego'}
          </button>
        </div>
      </div>
    </div>
